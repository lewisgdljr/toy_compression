...
struct centered_truncated_binary {
   ...
   static void encode( T x, T n, binary_io::bit_writer<Iterator>& storage ) {
     ...
     TOY_COMPRESSION_ASSERT(centered_truncated_binary, (1 <= x) && (x <= n));
...
struct binary_in_range {
   template <typename T, typename Iterator,
             typename = std::enable_if_t<std::is_unsigned_v<T>>>
   static void encode( T x, T low, T high, binary_io::bit_writer<Iterator>& storage )
  {
   ...
   template <typename T, typename Iterator, typename Iterator2,
             typename = std::enable_if_t<std::is_unsigned_v<T>>>
   static T decode( T low, T high, binary_io::bit_reader<Iterator, Iterator2>& storage )
  {
    TOY_COMPRESSION_ASSERT(binary_in_range, low >= 1);
    TOY_COMPRESSION_ASSERT(binary_in_range, low < high);
    return centered_truncated_binary::decode(high - low + 1, storage) + low - 1;
   }
};
