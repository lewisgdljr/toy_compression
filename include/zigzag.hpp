...
namespace integer_codes {
  ...
  {
    template<typename T>
    using enable_enc = std::enable_if_t<std::is_integral_v<T> &&
					std::is_signed_v<T>>;

    template<typename T>
    using enable_dec = std::enable_if_t<std::is_integral_v<T> &&
					std::is_unsigned_v<T>>;
  }
...
#undef BIT_HACK      
  ...
  struct offset_zigzag
  {
    ...
    template<typename T, typename = enable_dec<T>>
    constexpr static auto decode(const T x, const signed_of<T> offset) -> signed_of<T>
    {
